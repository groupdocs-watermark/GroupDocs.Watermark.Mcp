...
Assert-ServerJsonVersionMatchesDependencies `
    ...
    -ServerJsonPath ".\src\GroupDocs.Watermark.Mcp\.mcp\server.json" `
    -VersionPropertyName "GroupDocsWatermarkMcp"

exec { & dotnet restore src\GroupDocs.Watermark.Mcp.sln }

$isProd = $env:BUILD_TYPE -eq "PROD"

if ($isProd) {
    Write-Host "build: PROD build - stable version (no suffix)"
    exec { & dotnet build src\GroupDocs.Watermark.Mcp.sln -c Release --verbosity quiet --nologo }
} else {
    $commitHash = $(git rev-parse --short HEAD)
    $buildSuffix = "local-$commitHash"
    Write-Host "build: DEV build - version suffix is $buildSuffix"
    exec { & dotnet build src\GroupDocs.Watermark.Mcp.sln -c Release --version-suffix=$buildSuffix --verbosity quiet --nologo }
}
...
exec { & dotnet pack .\src\GroupDocs.Watermark.Mcp\GroupDocs.Watermark.Mcp.csproj @packArgs }
